...
const DetailPage = () => {
  ...
  const dataLoading = useSelector<RootState>((state) => state.detail.loading);
  const castCrewLoading = useSelector<RootState>(
    (state) => state.castCrew.loading
  );
  const reviewsLoading = useSelector<RootState>(
  ...
  return (
    <Fragment>
      {dataLoading || castCrewLoading || reviewsLoading ? (
        <Loading />
      ) : (
        <Box>
          {/* Background Section */}
